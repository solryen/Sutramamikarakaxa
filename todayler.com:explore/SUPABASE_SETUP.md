...
- Keep `VITE_SUPABASE_ADMIN_EMAIL=todaylerapp@gmail.com` locally so the UI and the Edge Function agree on which account may edit articles.
...
The browser still uses the `VITE_` vars in [.env.local](.env.local), but article writes go through the same-origin API proxy first, which then forwards to the Supabase Edge Function.
