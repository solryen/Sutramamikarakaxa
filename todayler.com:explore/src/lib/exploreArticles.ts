import { supabase, exploreSupabaseAnonKey, exploreSupabaseUrl, isSupabaseConfigured } from './supabase';
...
export async function fetchAllExploreArticles(): Promise<Article[]> {
  ...
  if (!exploreSupabaseUrl || !exploreSupabaseAnonKey) return [];

  const response = await fetch(`${exploreSupabaseUrl.replace(/\/$/, '')}/functions/v1/article-sync`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      apikey: exploreSupabaseAnonKey,
    },
