      ...
      <div className="w-full max-w-4xl mx-auto">
        ...
        {!session ? (
          <div className="mb-8 bg-amber-50 border border-amber-200 p-4 text-sm text-amber-900">
            {isGreek
              ? 'Το 8loop ανοίγει την admin διαδρομή. Ο σύνδεσμος επιβεβαίωσης θα σταλεί στο admin email.'
              : '8loop opens the admin bootstrap path. The confirmation link will be sent to the admin email.'}
            <div className="mt-3">
              <Link to="/explore/sign-in?next=/explore/dashboard&bootstrap=admin" className="underline font-medium">
                {isGreek ? 'Άνοιγμα admin bootstrap' : 'Open admin bootstrap'}
              </Link>
            </div>
          </div>
        ) : !isAdminEmail ? (
          <div className="mb-8 bg-amber-50 border border-amber-200 p-4 text-sm text-amber-900">
            ...
            <div className="mt-3">
              <Link to="/explore/sign-in?next=/explore/dashboard&bootstrap=admin" className="underline font-medium">
                {isGreek ? 'Επιβεβαίωση admin λογαριασμού' : 'Confirm admin account'}
              </Link>
            </div>
          </div>
        ) : null}
