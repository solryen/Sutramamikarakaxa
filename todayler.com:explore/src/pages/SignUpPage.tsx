...
export function SignUpPage() {
  ...
  const isAdminBootstrap = searchParams.get('bootstrap') === 'admin';

  const nextPath = useMemo(
    () => searchParams.get('next') || (isAdminBootstrap ? '/explore/dashboard' : '/explore'),
    [isAdminBootstrap, searchParams],
  );
  const isRecoveryLink = searchParams.get('mode') === 'recover';
  const recoveryModeActive = isRecoveryLink || passwordRecovery;

  const [mode, setMode] = useState<AuthMode>('signIn');
  const [email, setEmail] = useState(isAdminBootstrap ? exploreSupabaseAdminEmail : '');
  ...
  const [bootstrapLinkSent, setBootstrapLinkSent] = useState(false);
  ...
  useEffect(() => {
    if (isAdminBootstrap) {
      setMode('signIn');
      setEmail(exploreSupabaseAdminEmail);
    }
  }, [isAdminBootstrap]);
  ...
  useEffect(() => {
    if (!isAdminBootstrap || recoveryModeActive || authLoading || session || bootstrapLinkSent) {
      return;
    }

    if (!exploreSupabaseAdminEmail) {
      setError(
        isGreek
          ? 'Το admin email δεν έχει ρυθμιστεί.'
          : 'The admin email is not configured.',
      );
      return;
    }

    if (email.trim().toLowerCase() !== exploreSupabaseAdminEmail) {
      return;
    }

    const sendBootstrapLink = async () => {
      setLoading(true);
      setError('');
      setMessage('');
      ...
      const result = await sendMagicLink(
        ...
        buildRedirectUrl('/explore/sign-in', nextPath, { bootstrap: 'admin' }),
      );
      ...
      if (result.error) {
        ...
        setMessage(
          isGreek
            ? 'Στάλθηκε ο admin σύνδεσμος. Ελέγξτε το email σας και επιστρέψτε εδώ.'
            : 'Admin magic link sent. Check your inbox and return here.',
        );
        setBootstrapLinkSent(true);
      }

      setLoading(false);
    };

    void sendBootstrapLink();
  }, [authLoading, bootstrapLinkSent, email, isAdminBootstrap, isGreek, nextPath, recoveryModeActive, sendMagicLink, session]);
  ...
  const adminNote = isGreek
    ? 'Το 8loop ανοίγει την admin διαδρομή. Ο πραγματικός έλεγχος γίνεται μέσω Supabase magic link στο admin email.'
    : '8loop opens the admin bootstrap path. Real access is verified through a Supabase magic link sent to the admin email.';
  ...
  const submitSignIn = async (e: React.FormEvent) => {
    ...
    if (isAdminBootstrap) {
      ...
      if (result.error) {
        ...
        setMessage(
          isGreek
            ? 'Στάλθηκε ο admin σύνδεσμος. Ελέγξτε το email σας.'
            : 'Admin magic link sent. Check your email.',
        );
        setBootstrapLinkSent(true);
      }
    } else {
  ...
  const submitSignUp = async (e: React.FormEvent) => {
    ...
    const result = await signUpWithPassword(email, password, buildRedirectUrl('/explore/sign-in', nextPath));
    if (result.error) {
      setError(result.error);
  ...
  const submitResetRequest = async (e: React.FormEvent) => {
    ...
    const result = await sendPasswordResetEmail(email, buildRedirectUrl('/explore/sign-in?mode=recover', nextPath));
    if (result.error) {
      setError(result.error);
    } else {
      setMessage(
  ...
  const subtitle = recoveryModeActive
    ...
    : isAdminBootstrap
      ? isGreek
        ? 'Στείλτε το magic link στο admin email.'
        : 'Send the magic link to the admin email.'
    : mode === 'signUp'
      ? isGreek
  ...
  return (
    <div className="min-h-screen bg-stone-50 flex items-center justify-center p-6">
      <div className="w-full max-w-md bg-[#EAE7E0] border border-[#E5E1D8] p-10 shadow-sm relative">
        ...
        {!recoveryModeActive && !isAdminBootstrap && (
        ...
        {isAdminBootstrap && !recoveryModeActive && (
          <div className="mb-6 border border-stone-300 bg-white/70 p-4 text-xs text-stone-700 leading-relaxed">
            <div className="font-bold uppercase tracking-widest text-[10px] mb-2">
              {isGreek ? 'Admin Bootstrap' : 'Admin Bootstrap'}
            </div>
            <p>
              {isGreek
                ? 'Το 8loop ενεργοποίησε τη διαδρομή διαχείρισης. Θα σταλεί σύνδεσμος επιβεβαίωσης στο admin email.'
                : '8loop has opened the admin path. A confirmation link will be sent to the admin email.'}
            </p>
          </div>
        )}
        ...
        {recoveryModeActive ? (
          ...
          <form onSubmit={submitSignIn} className="space-y-4">
            <input
              ...
              className="w-full border border-stone-300 bg-white p-3 text-xs focus:border-stone-900 focus:outline-none placeholder:text-stone-400 text-stone-900"
              autoComplete="email"
              readOnly={isAdminBootstrap}
              disabled={isAdminBootstrap}
            />
            {!isAdminBootstrap && (
              <input
                type="password"
