  ...
  const handlePromoSubmit = async (e: React.FormEvent) => {
    ...
    if (promoCode === '8loop') {
      ...
      if (success) {
        setShowPromo(false);
        setPromoCode('');
        if (session?.user?.email?.toLowerCase() === exploreSupabaseAdminEmail) {
          navigate('/explore/dashboard');
          return;
        }

        const bootstrapTarget = '/explore/sign-in?next=/explore/dashboard&bootstrap=admin';
      ...
      <footer className="bg-stone-900 text-stone-50 py-16 mt-20 border-t border-stone-900">
        <div className="mx-auto max-w-7xl px-6 lg:px-10 flex flex-col md:flex-row items-start md:items-center justify-between gap-10">
          ...
          <div className="flex flex-wrap items-center gap-8 text-[11px] uppercase tracking-widest font-light">
            ...
            <Link to="/privacy" className="hover:text-orange-600 transition-colors">{isGreek ? 'Πολιτικη Απορρητου' : 'Privacy'}</Link>
            <Link to="/terms" className="hover:text-orange-600 transition-colors">{isGreek ? 'Οροι Χρησης' : 'Terms'}</Link>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="bg-stone-50 border border-stone-200 p-8 shadow-sm w-full max-w-sm relative">
            ...
            <p className="text-[10px] uppercase tracking-widest text-stone-500 font-medium text-center mb-5">
              {isGreek ? 'Το 8loop ανοίγει τη διαδρομή διαχείρισης' : '8loop opens the admin bootstrap path'}
            </p>
